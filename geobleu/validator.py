...
def check_consistency(pred_seq, ans_seq, uid):
    ...
    if len(pred_seq) != len(ans_seq):
        error(error_prefix + "The length doesn't match between the generated and reference trajectories.")
    ...
    for idx, (pred_step, ans_step) in enumerate(zip(pred_seq, ans_seq)):
        ...
        if not (pred_d == ans_d and pred_t == ans_t):
            error(
                error_prefix + "Day and time are not the same; "
...
def main():
    ...
    with gzip.open(generated_fpath, "rt", encoding="utf-8") as f:
        for i, l in tqdm(enumerate((f))):
            ...
            if len(cols) != 5:
                error(error_prefix + "The number of columns must be 5")

            # each column must be numeric
            for c in cols:
                if not c.isnumeric():
                    error(error_prefix + "Each column must be numeric")
            ...
            if d < d_min or d > d_max:
                error(error_prefix + "d={} is out of range".format(d))
            if t < t_min or t > t_max:
                error(error_prefix + "t={} is out of range".format(t))
            if x < coord_min or x > coord_max:
                error(error_prefix + "x={} is out of range".format(x))
            if y < coord_min or y > coord_max:
                error(error_prefix + "y={} is out of range".format(y))
    ...
    if uid_set != uid_set_ref:
        error("The set of uid's doesn't match that of reference; " "there seem to be extra or lacking uid's")
    print("")
